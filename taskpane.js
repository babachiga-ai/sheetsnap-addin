...
async function buildCanvasFromSelection(onProgress) {
  ...
  await Excel.run(async (context) => {
    ...
    onProgress(30, 'Reading cell styles…');

    // Queue per-cell font & fill loads (batched into the same sync)
    const cellRefs = [];
    for (let r = 0; r < rows; r++) {
      const rowRefs = [];
      for (let c = 0; c < cols; c++) {
    ...
    await context.sync(); // single round-trip for everything queued above
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        let fillColor = null;
        try { fillColor = cell.format.fill.color; } catch (_) {}
        ...
        if (text) {
          ...
          try {
            ...
            underline = cell.format.font.underline && cell.format.font.underline !== 'None';
            fontColor = cell.format.font.color || '#000000';
          ...
          let align = 'General';
          try { align = cell.format.horizontalAlignment || 'General'; } catch (_) {}
...
async function copyLastToClipboard() {
  ...
  try {
    ...
    showToast('⚠️ Clipboard blocked — use Download instead', 'err');
  }
}
