...
// Excel computes color-scale colors dynamically at render time; they
// are NOT stored as a simple per-cell property we can just read. So we
// read the actual rules (min/mid/max stops + their colors) and the
// range's real values, then reproduce Excel's own interpolation
// ourselves. This is what makes those "heat map" percentage columns
// (green → yellow → red) show up correctly.
// ─────────────────────────────────────────────────────────────────────
...
async function getColorScaleGrid(context, range, rows, cols) {
  // Each color-scale rule applies to its OWN range (e.g. just column F),
  // not the whole selection — mixing values across columns (percentages
  // with millions-scale targets, counts, etc.) corrupts the min/max scale
  // entirely. We must resolve each rule against only the cells it
  // actually governs, using absolute worksheet coordinates to align it
  // with our selection.
  range.load(['rowIndex', 'columnIndex']);

  const cfCollection = range.conditionalFormats;
  cfCollection.load('items,items/type');
  await context.sync();

  const colorScaleCFs = cfCollection.items.filter(cf => cf.type === Excel.ConditionalFormatType.colorScale);
  if (colorScaleCFs.length === 0) return null;

  const selRowStart = range.rowIndex;
  const selColStart = range.columnIndex;

  // Batch-load each rule's own range + criteria in one round trip
  const cfInfos = colorScaleCFs.map(cf => {
    const r = cf.getRange();
    r.load(['rowIndex', 'columnIndex', 'rowCount', 'columnCount', 'values']);
    cf.colorScale.load('criteria');
    return { cf, r };
  });
  await context.sync();
  ...
  function resolveThreshold(stop, dataMin, dataMax, sorted) {
    if (!stop) return null;
    const type = stop.type, formula = stop.formula;
    if (type === 'LowestValue') return dataMin;
    if (type === 'HighestValue') return dataMax;
    if (type === 'Number') return parseFloat(formula);
    if (type === 'Percent') return dataMin + (dataMax - dataMin) * (parseFloat(formula) / 100);
    if (type === 'Percentile') {
      const idx = Math.round((parseFloat(formula) / 100) * (sorted.length - 1));
      return sorted[Math.max(0, Math.min(sorted.length - 1, idx))];
    }
    return null; // Formula-based stops: unsupported, skip this rule
  }

  // Reverse order so the first-priority rule in Excel's own list ends up
  // as the final color where multiple rules happen to overlap.
  for (let i = cfInfos.length - 1; i >= 0; i--) {
    const { cf, r } = cfInfos[i];
    try {
      const cfRowStart = r.rowIndex, cfColStart = r.columnIndex;
      const cfRows = r.rowCount, cfCols = r.columnCount;
      const cfValues = r.values;

      // Thresholds computed ONLY from this rule's own range — not the
      // whole selection — so percentages, counts, and targets never mix.
      const nums = [];
      for (let rr = 0; rr < cfRows; rr++) {
        for (let cc = 0; cc < cfCols; cc++) {
          if (typeof cfValues[rr][cc] === 'number') nums.push(cfValues[rr][cc]);
        }
      }
      ...
      const crit = cf.colorScale.criteria;
      const minT = resolveThreshold(crit.minimum, dataMin, dataMax, sorted);
      const maxT = resolveThreshold(crit.maximum, dataMin, dataMax, sorted);
      const midT = crit.midpoint ? resolveThreshold(crit.midpoint, dataMin, dataMax, sorted) : null;
      if (minT === null || maxT === null || maxT <= minT) continue;

      const minColor = crit.minimum.color;
      const maxColor = crit.maximum.color;
      const midColor = crit.midpoint ? crit.midpoint.color : null;

      // Only paint cells where this rule's range actually overlaps our
      // captured selection, aligned via absolute worksheet coordinates.
      const overlapRowStart = Math.max(cfRowStart, selRowStart);
      const overlapRowEnd   = Math.min(cfRowStart + cfRows, selRowStart + rows);
      const overlapColStart = Math.max(cfColStart, selColStart);
      const overlapColEnd   = Math.min(cfColStart + cfCols, selColStart + cols);

      for (let wr = overlapRowStart; wr < overlapRowEnd; wr++) {
        for (let wc = overlapColStart; wc < overlapColEnd; wc++) {
          const localR = wr - selRowStart, localC = wc - selColStart; // our grid
          const cfR = wr - cfRowStart, cfC = wc - cfColStart;         // rule's own grid
          const v = cfValues[cfR][cfC];
          if (typeof v !== 'number') continue;

          let color;
          if (midColor && midT !== null) {
            if (v <= minT) color = minColor;
            else if (v >= maxT) color = maxColor;
            else if (v <= midT) color = lerpColor(minColor, midColor, (v - minT) / ((midT - minT) || 1));
            else color = lerpColor(midColor, maxColor, (v - midT) / ((maxT - midT) || 1));
          } else {
            if (v <= minT) color = minColor;
            else if (v >= maxT) color = maxColor;
            else color = lerpColor(minColor, maxColor, (v - minT) / ((maxT - minT) || 1));
          }
          grid[localR][localC] = color;
        }
      }
    } catch (e) {
      console.warn('[SheetSnap] Skipping one color-scale rule:', e.message);
    }
  }
...
async function buildCanvasFromSelection(onProgress) {
  ...
  await Excel.run(async (context) => {
    const range = context.workbook.getSelectedRange();
    range.load(['rowCount', 'columnCount', 'text', 'values']);
    await context.sync();
    ...
    onProgress(48, 'Resolving conditional formatting…');
    let colorScaleGrid = null;
    try {
      colorScaleGrid = await getColorScaleGrid(context, range, rows, cols);
    } catch (e) {
      console.warn('[SheetSnap] Color scale resolution skipped:', e.message);
    }
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        let fillColor = null;
        if (colorScaleGrid && colorScaleGrid[r][c]) {
          fillColor = colorScaleGrid[r][c]; // conditional-format color scale wins
        } else {
