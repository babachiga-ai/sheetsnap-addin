...
async function getColorScaleGrid(context, range, rows, cols, values) {
  ...
  colorScaleCFs.forEach(cf => cf.colorScale.load('criteria'));
  await context.sync();
  ...
  const dataMin = Math.min(...allNums), dataMax = Math.max(...allNums);
  const sorted = [...allNums].sort((a,b) => a-b);

  function resolveThreshold(stop) {
    if (!stop) return null;
  ...
  // Later conditional formats in the collection take priority (matches
  // Excel's own "top rule wins" behavior when rules overlap) — apply in
  // reverse so the first-priority rule ends up as the final color.
  for (const cf of [...colorScaleCFs].reverse()) {
    const crit = cf.colorScale.criteria;
    const minT = resolveThreshold(crit.minimum);
    const maxT = resolveThreshold(crit.maximum);
    const midT = crit.midpoint ? resolveThreshold(crit.midpoint) : null;
...
async function buildCanvasFromSelection(onProgress) {
  ...
  await Excel.run(async (context) => {
    ...
    try {
      colorScaleGrid = await getColorScaleGrid(context, range, rows, cols, range.values);
    } catch (e) {
